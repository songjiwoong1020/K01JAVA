...
//		}
		...
		/*
		조건검사후 실행할 문장이 하나인 경우에는 중괄호({})를 생략할수있다.
...
//		int ascillCode = System.in.read();//문자하나 입력 받아서 아스키코드값을 저장
//		System.out.println("입력한문자:"+ ascillCode);
//		System.out.println("입력한문자:"+ (char)ascillCode);
		...
		※숫자의 구간을 정할때는 반드시 논리AND(&&)를 사용해야 한다.
		 */
		if(ascillCode >= 48 && ascillCode <= 57) {
		...
		비교연산이 가능하다.
		 */
		boolean isNumber = (ascillCode>='0' && ascillCode<='9');
		if(isNumber==true) {
			System.out.println("입력한 문자는 숫자입니다.");
		}
		else {
