...
class B extends A {
	...
	public void loadMethod(int num) {
		System.out.println("Bd의 loadMethod");
	}
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		호출된다. 즉 아래 부분은 모두 C클래스에서 생성된 메소드가
		호출된다.
		 */
		...
		ref2.loadMethod();
		ref2.loadMethod(1);// -> B클래스까지 영역까지 접근가능
...
//		ref1.loadMethod(1.1);//접근불가
		...
		참조변수로 참조 할 수 없다. 자식타입의 참조변수로
		부모객체를 참조하는것은 불가능하다.
		 */
//		B refId2 = new refId1;
//		C refId3 = new refId1;
		
		//(미리보기) 아래와 같이 다운캐스팅(강제형변환)하면 참조 가능하다.
		C refId3 = (C)refId1;
	}
