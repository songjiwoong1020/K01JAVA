...
class Burger {
	...
	//인자생성자
	Burger(String burgerName, int price, String patty, String sauce, String vegetable){
...
class HamburgerPrice {
	...
	void price() {
		System.out.println("결재금액:"+(burger.getPrice()+COKE+POTATO));
	}
...
//		return total;
//	}
	//리턴으로 해서 자식클래스에서 오버라이딩될때 코드가 더 짧아질수 있음.
...
class SetPrice extends HamburgerPrice {	
	...
	void price() {
		//부모의 계산 결과에서 500 차감
		System.out.println("세트결재금액:"+(burger.getPrice()+COKE+POTATO-500));
	}
