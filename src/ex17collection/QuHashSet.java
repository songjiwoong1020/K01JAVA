...
set계열의 컬렉션은 기본적으로 중복저장을 허용하지 않는다. 그러나 아래와 같이 새롭게 정의한 클래스에 대해서는 equals(), hashCode() 메소드를 적절히 오버라이딩 처리해야 중복을 제거할 수 있다.
...
class Avengers {
	...
	public boolean equals(Object obj) {
		...
		if(compareAvengers.name.equals(this.name) && compareAvengers.heroName.equals(this.heroName) && compareAvengers.weapon.equals(this.weapon)) {
			return true;
		}else {
			return false;
		}
...
public class QuHashSet {
	...
	public static void main(String[] args) {
		...
		while(itr.hasNext()) {
			...
			if(search.equals(avengers.name)) {
				//검색결과가 있다면 searchFlag값 변경
				searchFlag = true;
				//toString()메소드를 오버라이딩 했으므로 객체를 즉시 출력 가능.
