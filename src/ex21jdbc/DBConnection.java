...
public class DBConnection {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException {
		...
		2] 오라클에 연결하기 위한 커넥션URL, 계정ID, 패스워드를 준비한다.
			...
			※서버환경에 따라 ip주소, 포트번호, sid는 변경 될 수 있다.
		 */
		...
		/*
		2-1] DriveManager클래스의 getConnection()메소드를 호출하여
		오라클에 연결 시도한다. 연결에 성공할 경우 연결된 주소를 반환한다.
		 */
		...
		if(con != null) {
			...
			while(rs.next()) {
				...
				쓸 수도 있다.
				 */
				String emp_id = rs.getString(1);//employee_id컬럼 가지고옴(DB는 인덱스가 1부터시작)
				String f_name = rs.getString("first_name");//컬럼명을 그대로 이용함(위에처럼 숫자(인덱스)로써도 무방한듯)
			...
			/*
			6] 자원반납 : 모든 작업을 마친 후에난 메모리 절약을 위해
				연결했던 자원을 반납한다.
			 */
